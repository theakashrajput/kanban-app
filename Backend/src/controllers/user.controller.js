...
import { changePasswordService, loginUserService, refreshAccessTokenService, registerUserService, updateUserProfileAvatarService, userLogoutService } from "../services/user.service.js";
...
export const loginUser = asyncWrapper(async (req, res) => {
    ...
    const { accessToken, refreshToken, updatedUser } = await loginUserService({ userName, email, password })

    return res
        .status(200)
        .cookie("accessToken", accessToken, cookieOptions)
        .cookie("refreshToken", refreshToken, cookieOptions)
        .json(new AppResponse(200, "User logged in successfully", updatedUser.toSafeObj()));
});
...
export const changePassword = asyncWrapper(async (req, res) => {
    ...
    if (!oldPassword || !newPassword) throw new AppError(400, "All fields are required");
...
export const updateProfileAvatar = asyncWrapper(async (req, res) => {
    ...
    const updatedUserProfile = await updateUserProfileAvatarService(user._id, avatar);

    return res
        .status(200)
        .json(new AppResponse(200, "Avatar image updated successfully", updatedUserProfile.toSafeObj()));
});